...
function decrypt_sm4_2(msg,key_tdh2) {  
  const key_SM4 = (key_tdh2.toString()).substring(0, 16)
  const real_key_SM4 = keyToAsciiArray(key_SM4)  
  const decmsg = sm4.decrypt(msg, real_key_SM4); 
...
function verify_share (cipher, pk, share_i) {
  ...
  let U = p256.decodePoint(Buffer.from(TDH2CtxtObject.U, 'base64'))
  let E_i = p256.decodeScalar(Buffer.from(share_i.E_i, 'base64'))
  let F_i = p256.decodeScalar(Buffer.from(share_i.F_i, 'base64'))
  let U_i = p256.decodePoint(Buffer.from(share_i.U_i, 'base64'))
  let index = share_i.Index
  //u_i_hat = u ^ f_i / u_i ^ e_i
...
function verify_Check (ct, pub) {
  // check generate w, w_bar
  // w = g*f - u*e
  let F = p256.decodeScalar(Buffer.from(ct.F, 'base64'))
  let E = p256.decodeScalar(Buffer.from(ct.E, 'base64'))
  let U = p256.decodePoint(Buffer.from(ct.U, 'base64'))
  let U_bar = p256.decodePoint(Buffer.from(ct.U_bar, 'base64'))
  let g_bar = p256.decodePoint(Buffer.from(pub.G_bar, 'base64'))

  let C = Buffer.from(ct.C, 'base64')
  let Label = Buffer.from(ct.Label, 'base64')
...
function combine_Share (pk, cipher, shares, t) {
  ...
  const TDH2CtxtBuffer = Buffer.from(TDH2CtxtBase64, 'base64')
  const TDH2CtxtObject = JSON.parse(TDH2CtxtBuffer.toString('utf8'))
  let C = Buffer.from(TDH2CtxtObject.C, 'base64')
  ...
  for (let i = 0; i < lambda_list.length; i++) {
    let U_i = p256.decodePoint(Buffer.from(shares[i].U_i, 'base64'))
...
function decrypt_SM4 (key_TDH2, cipher) {
  ...
  let C = Buffer.from(data.SymCtxt, 'base64')
...
function sm3Fun(buffermsg){
  return sm3(Buffer.from(buffermsg, 'hex'))
}

export  {sm3Fun,decrypt_SM4,encrypt_sm4,decrypt_sm4_2, decrypt_AESGCM, combine_Share, verify_share, encrypt, GenerateKeys, newPoly, ployShare, Combine, tdh2EncryptInner, tdh2Decryption, verifyShare, combineShare, p256,sm3hash,sm3hashStr}