...
export default function PercentIncrease({address, overview}) {
  ...
  const url = '/blockBrowser/leaderboard/weekperincrease'
  const weekTransactionsRequest = useRequest(
    body => ({
      url: url, 
      method: 'post',
  ...
  useEffect(() => {
    weekTransactionsRequest.run(state.body);
  }, [state]);

  if (weekTransactionsRequest.loading) {
    return <Loading />;
  }

  const data = weekTransactionsRequest?.data || [];
  const counts = weekTransactionsRequest.data?.length || 0; 
