...
export default function MonitorKafa({address, overview}) {
    ...
    if(type===1){  
      addMonitorRequest.run({...form}).then(res => {  
          ...
          if (res.code === 0) { 
            ...
            setIsShow(false); 
            queryList(true);
            setForm({id:0,tag_name:'',brokers:'',topic:''})
          }else{
  ...
  const addMonitorRequest = useRequest(
    body => ({
    url: '/chainBrowser/user/manager/addMonitorKafka', 
    method: 'post',
    headers: {
  ...
  const editMonitorRequest = useRequest(
    body => ({
    url: '/chainBrowser/user/manager/updateMonitorKafka', 
    method: 'post',
    headers: {
      'Authorization': user.token,
    },
  ...
  const delMonitorRequest = useRequest(
    body => ({
    url: '/chainBrowser/user/manager/deleteMonitorKafka', 
    method: 'post',
    headers: {
      'Authorization': user.token,
    },
  ...
  const url = '/chainBrowser/user/manager/allMonitorKafkas'
  const monitoryKafaRequest = useRequest(
        body => ({
        url: url, 
        method: 'post',
        body: JSON.stringify(body),
  ...
  const queryList=(flag)=>{
    if(flag){
      monitoryKafaRequest.run(state.body);
    }
  }
  ...
  if (monitoryKafaRequest.loading) {
    return <Loading />;
  }

  const data = monitoryKafaRequest?.data || [];  
  return (
